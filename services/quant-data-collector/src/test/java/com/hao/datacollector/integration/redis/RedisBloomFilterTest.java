...
public class RedisBloomFilterTest {
    ...
    public void testAddAndMightContain() {
        // 1⃣ 清理测试位图，确保布隆过滤器是空的
        redisConfig.del(BLOOM_KEY);
        logger.info("清理测试布隆过滤器_key:_{}", BLOOM_KEY);
        // 2⃣ 添加元素到布隆过滤器
        ...
        logger.info("日志记录|Log_message,添加元素到布隆过滤器:_{},_{}", element1, element2);
        // 3⃣ 校验已添加元素，应该返回 true
        ...
        logger.info("日志记录|Log_message,校验已添加元素是否存在:_{}_->_{},_{}_->_{}", element1, contains1, element2, contains2);
        // 4⃣ 校验未添加元素，应该返回 false（布隆过滤器可能误判为 true，但一般不会）
        String element3 = "user:9999";
        boolean contains3 = redisBloomFilter.mightContain(BLOOM_KEY, element3);
        logger.info("日志记录|Log_message,校验未添加元素是否存在:_{}_->_{}", element3, contains3);
        // 5⃣ 断言结果
